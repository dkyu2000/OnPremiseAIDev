...
| 에이전트가 개발 서버(Flask/Node 등) 백그라운드 기동 명령에서 응답 없이 멈춤 | OpenCode bash 도구가 `nohup ... &`로 정상 백그라운드 처리된 명령도 종료를 계속 기다리는 한계(2026-07-21~22 실측, 명령어 자체는 터미널 직접 실행 시 정상) | ★[2026-07-22 1차] AGENTS.md 지침 추가만으로는 불충분(지침 인지≠실행 시 준수, `nohup npm run dev` 등 2시간 동안 20회 재현). ★[2차] `permission.bash`에 `nohup */* &`만 deny 추가했으나 모델이 `&` 없이 **foreground로 `python app.py` 직접 실행**해 우회됨(문법이 아니라 실제 서버 기동 명령 자체를 막아야 함을 확인). ★[2026-07-22 3차, 확정] `python app.py`/`node index.js`/`npm run dev`/`flask run`/`uvicorn` 등 흔한 서버 진입점을 모두 deny로 확장 — 신규 세션에서 실행 시도 자체 없이 대안(별도 터미널/`subprocess.Popen`) 안내까지 실측 확인. **⚠ `permission` 설정은 이미 열려 있는 세션에 소급 적용되지 않음** — 설정 변경 후 열려 있는 OpenCode 창을 모두 재시작해야 함(`docs/USER_GUIDE.md` §3 참조) |
| 에이전트 응답이 한글/영어로 왔다갔다 함(AGENTS.md 언어 지침이 있는데도) | ①컨텍스트 자동요약(`compaction`)은 **native 전용 에이전트라 AGENTS.md(`instructions`)를 아예 안 받고** 자체 영어 하드코딩 프롬프트("respond in same language")만 사용 — 요약 대상이 영어 코드/로그 위주면 영어로 요약됨(2026-07-22 세션 export로 확인). ②Plan 모드·5만 토큰 이상 장문 세션에서는 지침을 받고도 모델이 확률적으로 이탈(개발 서버 문제와 동일 근본원인 — LLM 지침 순응은 확률적) | `opencode.jsonc`에 `agent.compaction.prompt`로 내장 프롬프트를 한국어 강제 문구 포함 버전(`compaction-ko-prompt.txt.example`)으로 교체(`opencode debug agent compaction`으로 반영 확인) — ①은 결정론적으로 해결. ②는 AGENTS.md에 "컨텍스트가 아무리 길어도 예외 없음" 문구 추가로 완화(완전 차단은 불가, 재현되면 "한국어로 답해줘" 재요청). `docs/USER_GUIDE.md` §3 참조 |
