...
> ★[2026-07-07 확정] 운영 구성은 **main + autocomplete 2-트랙만** 사용한다(§10 선택지 A). vLLM sub(8001)/gemma27b(8002)는
> Phase B/C 역사적 검증에만 쓰였고, 서브 채팅 모델 자체를 운영에서 쓰지 않기로 확정했으므로 **평소 기동하지 않는다.**
...
[LiteLLM 게이트웨이 :4000]  ── 가상키/RateLimit/PII(Presidio)/Audit/라우팅
        │                         │
        ├──► vLLM main   :8000   (Llama  — 채팅·에이전트·tool calling)      ★운영 상시 기동
        └──► vLLM autocomplete :8003 (StarCoder2 — tab 자동완성 FIM)        ★운영 상시 기동
...
| 추론(메인) | vllm-main | 8000 | Llama 채팅/에이전트 | 상시 |
| 추론(자동완성) | vllm-autocomplete | 8003 | StarCoder2 FIM | 상시 |
...
> ★**[2026-07-13 운영 결정 갱신] 선택지 E(신규, 아래)를 운영 구성으로 최종 채택했다.** 2026-07-08에 채택했던
> 선택지 D(Llama NVFP4+FIM 15B)는 안정적으로 운영됐으나, **아키텍처가 다른 `gpt-oss-120b`(OpenAI, 표준 MoE)로
> 품질 PoC에서 근소 우위(hard-set 7/7 vs 6.5/7) + 처리량 8배가 실측 확인**되어 E로 대체했다(완료보고서 §17).
> FIM은 VRAM 마진 확보를 위해 15B→7B로 하향(품질 트레이드오프 감수). 서브 채팅 모델(Gemma 트랙)을 쓰지
> 않는다는 결정(운영 단순성 우선, 2026-07-07)은 그대로 유지 — 선택지 B/C는 여전히 미채택.
> `scripts/switch_model_option.sh {a|d|e}`로 세 구성 중 언제든 재전환 가능.

**선택지 A — 2-트랙(FP8, 품질 최우선·안정) — ★[구 채택 2026-07-07, 2026-07-08 D로 대체됨]:**
...
**선택지 D — 2-트랙(main NVFP4 + FIM 15B, 서브 없음) — ★[구 채택 2026-07-08, 2026-07-13 E로 대체됨]:**
...
| 역할 | 모델 | 양자화 | 가중치(실측) | KV 여유(실측, 컨텍스트 27648/8192) |
|------|------|--------|------|------|
| 채팅·에이전트 | **gpt-oss-120b**(OpenAI, 128 전문가 중 4개 활성) | **MXFP4**(네이티브) | 65.97GiB | 6.46GiB, 동시성 5.23x |
| 자동완성(FIM) | StarCoder2-**7B**(15B에서 하향 — VRAM 확보) | FP8 | 6.96GiB | 2.39GiB, 동시성 ~4.8x |

**GPU 총사용 ~92.4GB/97.9GB(여유 ~5.4GB)** — D(여유 ~4.5GB)보다도 오히려 여유 있음. 실제 채택 파라미터(`.env`,
...
`MAIN_GPU_UTIL=0.80`, `MAIN_MAX_LEN=27648`, `AUTOCOMPLETE_MODEL_PATH=/models/starcoder2-7b-fp8`,
...
| 선택지 | 메인 | 성격 | 채택 여부 |
|--------|------|------|----------|
| A | Llama 70B-FP8 | 검증된 안정·빠름 | ⏮ 구 채택(2026-07-07) → D로 대체됨 |
...
| D | Llama 70B-**NVFP4** + FIM **15B** | A의 서브 없는 안정성 + NVFP4 절감분을 FIM 품질에 투자 | ⏮ 구 채택(2026-07-08) → E로 대체됨 |
| **E** | **gpt-oss-120b**(MXFP4) + FIM **7B** | 다른 아키텍처, 채팅/에이전트 품질·속도 우위 | ✅ **채택(운영 확정, 2026-07-13)** |

> ★[2026-07-13 확정] 운영은 **선택지 E만 사용한다.** 서브 채팅 모델(Gemma 트랙)은 앞으로 검증·운영 어느
> 단계에서도 사용하지 않기로 한 결정(2026-07-07)은 유지된다. A/D는 `scripts/switch_model_option.sh {a|d}`로
> 필요 시 언제든 재전환 가능(과거 검증 기록으로 보존).
