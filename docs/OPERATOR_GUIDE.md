...
> ★**[2026-07-08 운영 결정 갱신] 선택지 D(신규, 아래)를 운영 구성으로 최종 채택했다.** 2026-07-07에 채택했던
> 선택지 A(FP8 2-트랙)는 실장비 기동 결과 VRAM 여유가 30MiB까지 타이트해져, main만 **NVFP4로 전환**하고 그
> 절감분으로 **FIM을 StarCoder2-7B→15B로 상향**한 선택지 D로 대체했다. 서브 채팅 모델(Gemma 트랙)을 쓰지
...
**선택지 D — 2-트랙(main NVFP4 + FIM 15B, 서브 없음) — ★[운영 채택, 2026-07-08 실장비 실측]:**
...
| 선택지 | 메인 | 성격 | 채택 여부 |
|--------|------|------|----------|
| A | 70B-FP8 | 검증된 안정·빠름 | ⏮ 구 채택(2026-07-07) → D로 대체됨 |
| B | 70B-NVFP4 + 27B-FP8 서브 | 서브 모델 다양성·부하분산 | ❌ 미채택(서브 채팅 모델 자체를 쓰지 않기로 결정) |
| C | 123B-NVFP4 | 같은 VRAM 에 더 큰 모델 → 품질 상향 | ❌ 미채택(D로 충분, 추가 검증 부담 대비 실익 낮음) |
| **D** | 70B-**NVFP4** + FIM **15B** | A의 서브 없는 안정성 + B의 NVFP4 절감분을 FIM 품질에 투자 | ✅ **채택(운영 확정, 2026-07-08)** |

> ★[2026-07-08 확정] 운영은 **선택지 D만 사용한다.** 서브 채팅 모델(Gemma 트랙)은 앞으로 검증·운영 어느
> 단계에서도 사용하지 않기로 한 결정(2026-07-07)은 유지된다. C(123B 상향)는 D로 이미 충분한 여유(~4.5GB)와
> 개선된 FIM 품질을 확보했다고 판단해 보류 상태를 유지한다.
