...
## 10. 운영 모델 구성 (Phase C 검증 반영, 2026-06-29 / ★2026-07-07 최종 채택·실측 반영)

Phase C 실측 결과를 반영한 운영 권장 구성. SM120(5090) 검증분은 운영 RTX PRO 6000(동일 SM120)에 그대로 이전된다.

> ★**[2026-07-07 운영 결정 확정] 선택지 A를 운영 구성으로 확정 채택했다.** 선택지 B/C는 아래 검토했으나
> 서브 채팅 모델 자체를 쓰지 않기로 확정(운영 단순성·VRAM 여유 우선)했으므로 **미채택**. RTX PRO 6000 실장비에서
> 선택지 A를 실제 기동해 본 결과, 이론치(~77GB 사용/~19GB 여유)보다 **실사용량이 훨씬 큼**이 확인됐다 — 아래 실측 참조.

**선택지 A — 2-트랙 (품질 최우선, 안정) — ★운영 채택·실장비 기동 완료:**
...
| 선택지 | 메인 | 성격 | 채택 여부 |
|--------|------|------|----------|
| **A** | 70B-FP8 | 검증된 안정·빠름 | ✅ **채택(운영 확정, 2026-07-07)** |
| **B** | 70B-NVFP4 + 27B-FP8 서브 | 서브 모델 다양성·부하분산 | ❌ 미채택(서브 채팅 모델 자체를 쓰지 않기로 결정) |
| **C** | 123B-NVFP4 | 같은 VRAM 에 더 큰 모델 → 품질 상향 | ❌ 미채택(A로 충분, 추가 검증 부담 대비 실익 낮음) |

> ★[2026-07-07 확정] 운영은 **선택지 A만 사용한다.** 서브 채팅 모델(Gemma 트랙)은 앞으로 검증·운영 어느 단계에서도
> 사용하지 않기로 결정했으므로, B/C로의 전환은 계획하지 않는다. 실측 결과 A만으로도 VRAM 여유가 1.4GB로 이미
> 타이트해 추가 트랙을 얹을 물리적 여유도 없다.
...
- [x] **모델 교체** — 검증 모델(8B/7B) → 운영 모델(70B/FIM)로 `.env` `MAIN_MODEL_PATH`·`docker-compose` `phase-prod` 프로파일 갱신, 사전 스테이징+게이트 완료(2026-07-07). ★서브(27B/9B)는 운영 미채택이라 교체 대상에서 제외.
