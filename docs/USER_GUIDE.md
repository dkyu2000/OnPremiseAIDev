...
- `opencode.json`의 `context` 값이 서버 실제 한도(`27648`)와 정확히 같으면, 스킬 프롬프트+대화 이력이
  조금만 늘어나도 남은 토큰 계산이 음수가 되어 발생합니다(2026-07-13 실측 확인·수정됨). `opencode.json`의
  `models.main-gptoss.limit.context`를 `27648`보다 여유 있게(예: `26000`) 낮추면 해결됩니다 — 배포용
  `opencode.json.example`에는 이미 반영되어 있습니다. 계속 발생하면 대화를 새로 시작(컨텍스트 초기화)하거나
  운영팀에 문의하세요.
