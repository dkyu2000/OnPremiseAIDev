...
### ⚠ 주의사항: 개발 서버(Flask/Node 등)를 직접 백그라운드로 켜라고 시키지 마세요
...
증상: 몇 분씩 응답이 없다가 결국 타임아웃되거나, 직접 `esc`로 중단해야 함. 반복되면 컨텍스트도
낭비됩니다(매번 실패한 시도가 대화 이력에 쌓임).

**권장 해결책**: 서버를 대화형으로 직접 켜라고 시키지 말고, **테스트 코드가 서버 생명주기를
프로그래밍적으로 관리**하도록 지시하세요.
- Python/pytest: `subprocess.Popen`으로 기동 + 헬스체크 폴링(`requests.get` 재시도) 후 테스트 진행,
  `fixture` 종료 시 `os.killpg`로 정리.
- Node/Jest: `child_process.spawn`을 `beforeAll`에서 호출 + 포트 응답 폴링, `afterAll`에서 종료.
...
```
한 번 이렇게 테스트 하네스를 만들어두면, 이후로는 `npm test`/`pytest` 한 번 실행으로 서버 기동부터
검증까지 끝나서 이 문제 자체가 재발하지 않습니다.
