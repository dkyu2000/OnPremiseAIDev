...
### 기존 프로젝트에 결함 수정·UX 개선을 시킬 때

"결함이 많고 화면 구성이 안 좋으니 고쳐줘" 같은 큰 지시 하나를 던지면 대체로 결과가 실망스럽습니다.
아래는 이미 어느 정도 진행된 프로젝트(개발이 몇 세션 이상 누적된 상태)를 정리·개선시킬 때 실사용으로
확인된 순서입니다.

**0단계 — Git부터 확보하세요(OpenCode에게 시키지 말고 직접).** 여러 세션에 걸쳐 OpenCode로 개발한
프로젝트는 git 저장소가 아닌 경우가 의외로 흔하고, 루트에 `debug*.js`, `*.log`, `*.bak`, 스크린샷
파일 같은 임시 산출물이 정리 안 된 채 쌓여 있는 경우도 많습니다(OpenCode가 디버깅 중 만든 파일을
스스로 치우지 않기 때문). 이 상태로 광범위한 수정을 시키면 무엇이 왜 바뀌었는지 추적도, 되돌리기도
안 됩니다. `git init` 후 OpenCode에게 정리를 맡기세요:
```
루트의 debug*.js, *.log, backend.pid, 스크린샷 등 임시 디버깅 산출물을 찾아서 정리하고,
node_modules/build 산출물/DB 파일 등을 제외하는 .gitignore를 만들어줘. 그리고 지금 상태로
git 초기 커밋을 만들어줘.
```

**1단계 — 먼저 진단만 시키세요(수정 금지).** 바로 고치라고 하면 모델이 눈에 띄는 것부터 손대다
정작 중요한 문제를 놓치기 쉽습니다. 목록부터 뽑게 하세요:
```
지금부터 코드를 고치지 말고 조사만 해줘. 프론트/백엔드 전체를 훑고, Playwright로 실제 화면을
조작하면서 다음을 표로 정리해줘: (1) 클릭해도 반응 없는 UI 요소, (2) 콘솔/네트워크 탭 에러,
(3) 기존 테스트 실행 결과 중 실패하는 케이스와 원인, (4) 이미 문서화됐지만 반영 안 된 개선사항.
결과는 docs/BUG_AUDIT.md 파일로 만들어줘.
```
- ⚠ 채팅 모델(main-nemotron/gpt-oss 등)은 **비전(이미지 인식)을 지원하지 않습니다.** 스크린샷을
  "보고 판단"하게 시키면 안 되고, Playwright의 DOM/접근성 트리 조회나 실제 클릭 결과(URL 변화,
  텍스트 변화, 네트워크 요청)로 판단하게 해야 합니다.
- 이 단계에서 "버튼을 눌러도 페이지 전환이 안 됨(라우터 연결 누락)"처럼 겉보기엔 사소해 보이지만
  실제 UX 불만의 원인인 배선 누락이 자주 발견됩니다.

**2단계 — OpenSpec으로 변경안을 먼저 문서화하세요(설치 시 함께 제공).** OpenSpec이 설치돼 있다면
(§3 "확장 기능" 참조) `/opsx-propose`로 무엇을 어떻게 고칠지 제안서부터 쓰게 하고, **직접 읽어보고**
`/opsx-apply`로 실행시키세요. **버그 수정과 UX 개편은 서로 다른 change로 분리**하는 걸 권장합니다 —
한 번에 섞으면 나중에 무엇이 왜 바뀌었는지 리뷰하기 어렵습니다.

**3단계 — UX는 "예쁘게" 대신 구체적으로 지시하세요.** 모델 입장에서 "보기 편하게 해줘"는 너무
모호해서 결과가 들쭉날쭉합니다. 레이아웃 패턴, 상태(로딩/에러/빈 목록) 처리, 어떤 컴포넌트를 어떻게
바꿀지를 구체적으로 지정할수록 결과가 안정적입니다:
```
목록 화면은 카드형 레이아웃으로, 항목이 없을 때는 안내 문구와 추가 버튼을 보여주는 empty state를
넣어줘. 폼 제출/삭제 시 로딩 상태와 성공/실패 토스트도 추가해줘.
```

**4단계 — 세션은 짧게, 단계마다 커밋하세요.** 큰 작업을 한 세션에 몰아넣지 말고, 한 단계가 끝날
때마다 git commit 후 새 세션(또는 `/compact`)으로 넘어가세요. 컨텍스트가 5만 토큰을 넘어가면 위
"응답이 한글/영어로 왔다갔다 하는 문제"에서 설명한 지침 이탈 가능성도, 전반적인 응답 품질 저하
가능성도 함께 올라갑니다.
