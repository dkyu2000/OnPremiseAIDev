...
> ⚠ **`context7`, `neon` MCP는 사용하지 마세요.** `context7`은 자체 호스팅이 불가능해 요청마다 외부
> 클라우드 API를 호출합니다(폐쇄망에서 동작 불가). `neon`은 Neon사의 클라우드 Postgres 전용 관리
> 도구로, 우리는 자체 호스팅 PostgreSQL을 쓰므로 애초에 해당 사항이 없습니다(CLAUDE.md §2/§6).
