...
| `option-g.env` | 선택지 G(2026-07-14~) — **gpt-oss-120b MXFP4 단독**(FIM 없음), 컨텍스트 65536, GPU 여유 ~6.05GB. 2026-07-13~21 운영 기본값이었으나 이후 L로 전환됨 |
| `option-l.env` | 선택지 L(2026-07-21~, **현재 라이브**) — **NVIDIA Nemotron-3-Super-120B-A12B NVFP4 단독**(FIM 없음), 컨텍스트 131072. OpenCode 실전검증 완료(발견 버그 4건 조치, `docs/INTERIM_REPORT.md` §3.1.1) |
| `option-s.env` | 선택지 S(2026-07-21~) — **Gemma 4-31B-it FP8 단독**(FIM 없음), 컨텍스트 65536. ⚠**미완료**: 프로파일 파일만 생성됐고 `switch_model_option.sh s`로 실제 전환 검증된 적 없음(§19.8은 격리 PoC일 뿐) — 전환 전 실측 재확인 필요 |

상세 비교는 `docs/VALIDATION_COMPLETION_REPORT.md` §13.9(A vs D), §17(D vs E), §18.13(G), §19.8(L/S/Qwen3.6 격리검증), §19.12(L 실전검증) 참조.
